...
def rnn_decoder_with_attention(decoder_inputs, initial_state, cell, loop_function,attention_states,scope=None):#3D Tensor [batch_size x attn_length x attn_size]
    ...
    with tf.variable_scope(scope or "rnn_decoder"):
        ...
        for i, inp in enumerate(decoder_inputs):#循环解码部分的输入。如sentence_length个[batch_size x input_size]
            ...
            ####combined_vector=tf.concat([state,context_vector],axis=1) #[batch_size x attn_size*2]
            #####W_c = tf.get_variable("W_c", shape=[hidden_size*2, hidden_size],initializer=tf.random_normal_initializer(stddev=0.1))  # [hidden_size,1]
            #####combined_vector=tf.tanh(tf.matmul(combined_vector,W_c)) #[batch_size x attn_size]
            ####print("combined_vector:",combined_vector,";inp:",inp)
            #####output, state = cell(inp,combined_vector ) #state, context_vector
            output, state = cell(inp, state,context_vector) #alternative:combine context vector(context_vector) with current hidden state(state) to yield the final attention vector.
